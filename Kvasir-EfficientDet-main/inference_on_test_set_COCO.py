...
parser.add_argument("-if", "--image_folder", type=str, default="/storage/student4/chau/dl_project/kvasir-seg/Kvasir-SEG/test/images")
...
parser.add_argument("-wf", "--weight_file", type=str, default="/storage/student4/chau/dl_project/EndoCV2021-EfficientDet-Pytorch-main/EndoCV2021-EfficientDet-Pytorch-main/logs/fold_0/efficientdet-d0_best.pth")
parser.add_argument("-rf", "--result_file", type=str, default="KVASIR_DATA1")
...
for img_path in img_paths:
    ...
    for i in range(len(ori_imgs)):
        ...
        for j in range(len(out[i]['rois'])):
            ...
            # Adding label with confidence score
            label = f'{obj}: {score:.2f}'
            cv2.putText(ori_imgs[i], label, (x1, y1-10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)

        # Save the visualized image
        visualized_image_path = f'visualized_{img_path.split("/")[-1]}'
        cv2.imwrite(visualized_image_path, ori_imgs[i])
...
# Utility function to convert numpy types to native types
def convert_to_native(data):
    if isinstance(data, dict):
        ...
        return data.item()  # Convert to a native Python type
    else:
        return data

# Save annotations to JSON
with open(inference_result_name + ".json", "w") as outfile:
    json.dump(convert_to_native(annotations), outfile)
